// src/pages/responsable/pointages.tsx
...
export default function ResponsablePointages() {
  ...
  const handleArrival = async () => {
    try {
      ...
      toast({ title: t('common.success'), description: t('responsable.pointages.arrival_success') });
      queryClient.invalidateQueries({ queryKey: ["pointages"] });
      setIsArrivalModalOpen(false);
    } catch {
      toast({ title: t('common.error'), description: t('responsable.pointages.save_error'), variant: "destructive" });
    }
  };
  ...
  const handleDepart = async () => {
    ...
    try {
      await updateDepart.mutateAsync({ id: pointageAujourdhui.id, heureDepart: departTime });
      toast({ title: t('common.success'), description: t('responsable.pointages.depart_success') });
      queryClient.invalidateQueries({ queryKey: ["pointages"] });
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-serif font-bold bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">
            {t('responsable.pointages.title')}
          </h1>
          <p className="text-primary/60 text-sm mt-1">{t('responsable.pointages.subtitle')}</p>
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          {!aDejaArrive ? (
            <Button onClick={() => setIsArrivalModalOpen(true)} className="gap-2 shadow-md">
              <LogIn className="h-4 w-4" /> {t('responsable.pointages.clock_in')}
            </Button>
          ) : !aDejaDepart ? (
            <Button onClick={() => setIsDepartModalOpen(true)} className="gap-2 shadow-md">
              <LogOut className="h-4 w-4" /> {t('responsable.pointages.clock_out')}
            </Button>
          ) : (
            <div className="text-sm text-muted-foreground bg-primary/5 px-4 py-2 rounded-lg">
              {t('responsable.pointages.already_done_today')}
            </div>
          )}
        </div>
          ...
          <Card className="border-primary/10 shadow-md rounded-2xl">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-primary/70">
                {t('responsable.pointages.days_worked_this_month')}
              </CardTitle>
              <TrendingUp className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-primary">{joursTravailles}</div>
              <p className="text-xs text-muted-foreground mt-1">{t('responsable.pointages.out_of_month', { total: new Date().getDate() })}</p>
            </CardContent>
          </Card>
          <Card className="border-primary/10 shadow-md rounded-2xl">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-primary/70">
                {t('responsable.pointages.total_hours')}
              </CardTitle>
              <Clock className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-primary">{totalHeures.toFixed(1)} h</div>
              <p className="text-xs text-muted-foreground mt-1">{t('responsable.pointages.hours_this_month')}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="border-primary/10 shadow-md rounded-2xl overflow-hidden">
          <CardHeader className="bg-primary/5 border-b border-primary/10 flex flex-row justify-between items-center">
            <CardTitle className="text-primary">{t('responsable.pointages.recent_attendance')}</CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              <div className="p-8 text-center text-muted-foreground">{t('common.loading')}</div>
            ) : derniersPointages.length === 0 ? (
              <div className="p-8 text-center text-muted-foreground">{t('responsable.pointages.no_attendance')}</div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-muted/50">
                    <tr>
                      <th className="text-left p-4 text-sm font-medium text-primary/70">{t('responsable.pointages.date')}</th>
                      <th className="text-left p-4 text-sm font-medium text-primary/70">{t('responsable.pointages.arrival')}</th>
                      <th className="text-left p-4 text-sm font-medium text-primary/70">{t('responsable.pointages.departure')}</th>
                      <th className="text-left p-4 text-sm font-medium text-primary/70">{t('responsable.pointages.hours')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <AnimatePresence>
        ...
        <Dialog open={isArrivalModalOpen} onOpenChange={setIsArrivalModalOpen}>
          <DialogContent className="sm:max-w-[400px] rounded-2xl border border-border shadow-2xl">
            <DialogHeader>
              <DialogTitle className="text-primary">{t('responsable.pointages.clock_in')}</DialogTitle>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label>{t('responsable.pointages.date')}</Label>
                <Input type="date" value={arrivalDate} onChange={(e) => setArrivalDate(e.target.value)} />
              </div>
              <div className="grid gap-2">
                <Label>{t('responsable.pointages.arrival_time')}</Label>
        ...
        <Dialog open={isDepartModalOpen} onOpenChange={setIsDepartModalOpen}>
          <DialogContent className="sm:max-w-[400px] rounded-2xl border border-border shadow-2xl">
            <DialogHeader>
              <DialogTitle className="text-primary">{t('responsable.pointages.clock_out')}</DialogTitle>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                <Label>{t('responsable.pointages.departure_time')}</Label>
