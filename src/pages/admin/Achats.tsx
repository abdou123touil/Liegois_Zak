...
import { useListAchats, useCreateAchat, useListFournisseurs, useListMatieresPremieres } from "@/lib/api-client";
...
export default function Achats() {
    ...
    const { data: matieres = [] } = useListMatieresPremieres();
    const createMutation = useCreateAchat();

    // Met à jour l’unité de mesure quand la matière change
    useEffect(() => {
        const selected = matieres.find(m => m.id.toString() === matiereId);
        if (selected) {
            setUniteMesure(selected.uniteMesure);
        } else {
            setUniteMesure("");
        }
    }, [matiereId, matieres]);
                                            ...
                                            {achats.map((achat, idx) => (
                                                ...
                                                >
                                                    <td className="p-4 font-medium text-primary/90 flex items-center gap-2">
                                                        ...
                                                        {achat.matierePremiere?.nom || achat.nomProduit}
                                                    </td>

                                                    <td className="p-4 text-primary/70">
                                                        {achat.quantite} {achat.uniteMesure}
                                                    </td>
                                                    ...
                                                    <td className="p-4 font-semibold text-primary">
                                                        {formatCurrency(achat.prixTotal)}
                                                    </td>
                ...
                <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
                    ...
                    >
                        ...
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid gap-2">
                                <Label>{t('achats.matiere')}</Label>
                                <Select value={matiereId} onValueChange={setMatiereId}>
                                    <SelectTrigger className="bg-card">
                                        <SelectValue placeholder={t('achats.select_matiere')} />
                                    </SelectTrigger>
                                    <SelectContent className="bg-card">
                                        {matieres.map(m => (
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="grid gap-2">
                                    ...
                                    <Input value={uniteMesure} disabled />
                                </div>
                            </div>
                            <div className="grid gap-2">
